...
const MODULOS = [
  { label: 'Mis notas', icon: '📝' },
  { label: 'Mi horario', icon: '🗓️' },
  { label: 'Mi asistencia', icon: '✅' },
  { label: 'Mis boletines', icon: '📄' },
  { label: 'Calendario escolar', icon: '📅' },
];

export default function HomeEstudiante({ navigation }) {
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 20 }}>
      <Text style={styles.titulo}>Panel del Estudiante</Text>
      {MODULOS.map((m) => (
